...
function Smooth(time, alpha)
	...
	return Color(100 + math.abs(math.sin(-RealTime() * 3.14 * time) * 155), 100 + math.abs(math.sin(RealTime() * 2.71 * time) * 155), 100 + math.abs(math.sin(-RealTime() * 6 * time) * 155), alpha or 255)
end

local CurColorID = 1
local CurrentColor = Color(0, 0, 0)
local NiceColors = {Color(255, 0, 0), Color(0, 255, 0), Color(0, 0, 255), Color(0, 255, 255), Color(255, 0, 255), Color(255, 255, 0)}
...
function SmoothTimer()
	...
	if not (math.abs(CurrentColor.r) >= math.abs(nextColor.r) and math.abs(CurrentColor.g) >= math.abs(nextColor.g) and math.abs(CurrentColor.b) >= math.abs(nextColor.b)) then
...
function TweenColor(a, b, coef, alpha)
	return Color(a.r + (b.r - a.r) * coef, a.g + (b.g - a.g) * coef, a.b + (b.b - a.b) * coef, alpha)
end
