...
function FMeta:TestPoint(p)
	...
	for k, v in pairs(self.txplanes) do
		if _spoint:Dot(v.normal) > v.dist then return false end
	end
...
geometry = {
	points = {Vector(-1, -1, -1), Vector(1, -1, -1), Vector(1, 1, -1), Vector(-1, 1, -1), Vector(-1, -1, 1), Vector(1, -1, 1), Vector(1, 1, 1), Vector(-1, 1, 1)},
	quads = {{4, 3, 2, 1}, {1, 2, 6, 5}, {2, 3, 7, 6}, {3, 4, 8, 7}, {4, 1, 5, 8}, {5, 6, 7, 8}},
	indices = {{1, 2}, {2, 3}, {3, 4}, {4, 1}, {5, 6}, {6, 7}, {7, 8}, {8, 5}, {1, 5}, {2, 6}, {3, 7}, {4, 8}},
	planeIndices = {
		right = {4, 5, 1},
		left = {2, 7, 3},
		bottom = {1, 6, 2},
...
if PROFILE_TIMINGS then
	...
	for i = 1, 100000 do
		myFrustum:Orient(pl:EyePos(), pl:EyeAngles())
	end
	...
	for i = 1, 1 do
		elist = myFrustum:CullEntities(ents.GetAll())
	end
	...
	for i = 1, 10000 do
		myFrustum:Setup(100, 800, 90, 1280, 720)
	end
