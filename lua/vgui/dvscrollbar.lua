...
function PANEL:OnMouseWheeled( dlta )
	...
	return self:AddScrollForce( dlta * -2 )
...
function PANEL:Think()
	local sf = self.ScrollForce or 0
	if not sf then return end
	if sf==0 then sf=nil return end
	
	local ft = FrameTime()
	ft = ft*10
	ft=1 - (ft>.999 and .999 or ft)
	
	local new_sf = sf* ft
	
	local d = sf-new_sf
	
	self.ScrollForce = new_sf
	
	if math.abs(new_sf)<0.0001 then 
		d = new_sf
		new_sf = nil
	end
...
function PANEL:AddScrollForce(n)
	self.ScrollForce = (self.ScrollForce or 0) + n
end
